...
export default function CeoRaffleDashboard() {
  ...
  const adminAction = useCallback(
    async (action: string, extra: Record<string, unknown> = {}) => {
      const res = await fetch(`/api/raffle/admin/${slug}`, {
        method: "POST",
  ...
  if (notFound) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-3 text-center px-4">
        ...
        <Link href="/ceo/dashboard" className="text-line underline text-sm">
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-10 pb-16">
      <div className="flex items-center justify-between mb-6 lg:mb-8">
        <div>
          <Link href="/ceo/dashboard" className="text-mut text-[11px] underline">
            ← Todos tus sorteos
          </Link>
